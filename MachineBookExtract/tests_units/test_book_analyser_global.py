...
class test_book_analyser_global(unittest.TestCase):

    def test_book_analise_chars_amount(self):
        book_expected = book_analyser_global("TEST CHRISTMAS CAROL")
        book_expected.book_chars_amount = 158965
        book_expected.book_words_amount = 28970

        book_expected.book_sentences_amount = 2006
        book_expected.book_sentences_average_chars = 77.48
        book_expected.book_sentences_average_words = 14.67

        book_expected.fre = 85.92
        book_expected.fog = 9.2
        book_expected.smog = 4.6

        book_expected.total_vb = 4972
        book_expected.present_vb = 1874
        book_expected.past_vb = 3098
        book_expected.present_vb_p = 37.69106999195495
        book_expected.past_vb_p = 62.30893000804505

        book_expected.dialogues_amount = 685
        book_expected.dialogues_long_a = 110
        book_expected.dialogues_short_a = 575
        book_expected.dialogues_long_p = 16.06
        book_expected.dialogues_short_p = 83.94

        book_expected.total_adj = 2127

        book_expected.characters = ['Marley', 'Weather', 'Groom', 'Grocer', 'Vaughan', 'Scrooge', 'Uncle', 'Bedlam',
                                    'Cornhill',
                                    'Prophet', 'Star', 'Ebenezer', 'Valentine', 'Orson', 'Fezziwig', 'Cratchit', 'Gale',
                                    'Jones', 'Dickens', 'Marleys',
                                    'Pauls', 'Speak', 'Ding', 'Baba', 'Crusoe', 'Welch', 'Wilkins', 'Dick', 'Robinson',
                                    'Fezziwigs']

        content = '''The Project Gutenberg EBook of A Christmas Carol, by Charles Dickens
                ...
                ***** This file should be named 30368-8.txt or 30368-8.zip *****
                This and all associated files of various formats will be found in:
                        https://www.gutenberg.org/3/0/3/6/30368/
        ...
        book_analise = book_analyser_global(content)
        book_analise.start()

        self.assertEqual(book_analise.book_chars_amount, book_expected.book_chars_amount)
        self.assertEqual(book_analise.book_words_amount, book_expected.book_words_amount)
        self.assertEqual(book_analise.book_sentences_amount, book_expected.book_sentences_amount)
        self.assertEqual(book_analise.book_sentences_average_chars, book_expected.book_sentences_average_chars)
        self.assertEqual(book_analise.book_sentences_average_words, book_expected.book_sentences_average_words)
        self.assertEqual(book_analise.fre, book_expected.fre)
        self.assertEqual(book_analise.fog, book_expected.fog)
        self.assertEqual(book_analise.smog, book_expected.smog)
        self.assertEqual(book_analise.total_vb, book_expected.total_vb)
        self.assertEqual(book_analise.present_vb, book_expected.present_vb)
        self.assertEqual(book_analise.past_vb, book_expected.past_vb)
        self.assertEqual(book_analise.present_vb_p, book_expected.present_vb_p)
        self.assertEqual(book_analise.past_vb_p, book_expected.past_vb_p)
        self.assertEqual(book_analise.dialogues_long_a, book_expected.dialogues_long_a)
        self.assertEqual(book_analise.dialogues_short_a, book_expected.dialogues_short_a)
        self.assertEqual(book_analise.dialogues_long_p, book_expected.dialogues_long_p)
        self.assertEqual(book_analise.dialogues_short_p, book_expected.dialogues_short_p)
        self.assertEqual(book_analise.total_adj, book_expected.total_adj)
        self.assertEqual(book_analise.characters, book_expected.characters)
